...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
       ...
       %      if(offset==1)
       %          contain(:,100) = [sqrt(I_E * I_E + Q_E * Q_E)];
       %      else
       %          contain(:,[100-(offset-1),100+(offset-1)]) = [sqrt(I_E * I_E + Q_E * Q_E),sqrt(I_L * I_L + Q_L * Q_L)];  
       %      end
       % 
       % 
       % end
       %             figure;plot(c2,contain);
...
L1 = sqrt(trackResults(1).I_L.^2 + trackResults(1).Q_L.^2);
